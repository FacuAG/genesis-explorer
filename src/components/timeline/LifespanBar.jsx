...
export function LifespanBar({ people = [], onSelectPerson, selectedPersonId, bookTitle = 'Génesis' }) {
  ...
  const minAM = hasPatriarchBars ? patriarchsWithDates[0].chronology.birth_am : 0;
  const maxAM = hasPatriarchBars ? (patriarchsWithDates[patriarchsWithDates.length - 1].chronology.death_am || 2369) : 2369;
  ...
  return (
    <div className="lifespan-container">
      ...
      {/* Escala Superior de Años AM (Solo si hay personajes con longevidad patriarcal en AM) */}
      {hasPatriarchBars && (
        <>
          <div className="lifespan-axis">
            <span style={{ left: '0%' }}>AM {minAM}</span>
            <span style={{ left: '25%' }}>AM {Math.round(minAM + totalSpan * 0.25)}</span>
            <span style={{ left: '50%' }}>AM {Math.round(minAM + totalSpan * 0.50)}</span>
            <span style={{ left: '75%' }}>AM {Math.round(minAM + totalSpan * 0.75)}</span>
